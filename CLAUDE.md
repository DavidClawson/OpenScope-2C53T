...
- **Buttons:** 15 physical buttons — 4x3 bidirectional GPIO matrix (PA7/PB0/PC5/PE2 × PA8/PC10/PE3) + 3 passive (PC8=POWER, PB7=PRM, PC13=UP). 14/15 hardware-confirmed; PRM (PB7) fix applied 2026-04-04 (pull-down config), pending bench verification. TMR3 ISR at 500Hz.
...
- **Buttons: 14/15 HARDWARE CONFIRMED + PRM fix pending verification.** Bidirectional 4x3 matrix + 3 passive. See `analysis_v120/button_map_confirmed.md` for complete mapping. PRM (PB7) root cause identified 2026-04-04: was configured pull-up (from `fulltest2.c`) so idle always read HIGH and press couldn't be detected. Fix: pull-down config in `button_scan.c:button_scan_init`, matching the working `btntest2.c:176`. Builds clean. Needs bench press to mark confirmed.
...
- **Button input: 14/15 HARDWARE CONFIRMED + PRM fix pending verification** — bidirectional 4x3 matrix scan at 500Hz via TMR3 ISR. Rows: PA7,PB0,PC5,PE2. Cols: PA8,PC10,PE3. Passive: PC8(POWER),PB7(PRM),PC13(UP). Complete mapping in `analysis_v120/button_map_confirmed.md`. PRM pull-down fix applied 2026-04-04 — needs bench press to confirm.
