...
- **Meter reading pipeline working** — first live readings 2026-04-03. The current decoder extracts cross-byte BCD digits from USART2 data frames, runs a stock-style meter display state layer, rejects wrong frame families, gates AC modes on AC evidence, and applies DCV range multipliers selected by stock-analysis bits (`frame[8].7`, `frame[3].4`, `frame[4].4`, `frame[5].4`). Live fixtures on this branch include 1.5V low-DCV (`frame[8].7` calibrated band), 5V DCV, 32V DCV, and high-voltage/mains-class frames. See `analysis_v120/meter_math_pipeline_annotated.c`, `analysis_v120/meter_acv_stock_case_2026_06_05.md`, and `src/drivers/meter_data.c`.
...
- **Meter DCV range status (2026-06-05):** The older 7V/11V limitation has been superseded for known voltage fixtures. DCV no longer derives range from `frame[6]` latch behavior or bytes `[10..11]`; it uses the stock-analysis range-bit priority. Live 32V frames with `frame[4].4` render about `31.96..31.98 V`, 5V frames with `frame[3].4` render about `4.994 V`, and low-DCV frames with `frame[8].7` render about `1.497..1.500 V` using the temporary per-unit coefficient. The long-term calibration source is still unresolved: replay/load the stock factory calibration block instead of widening hardcoded coefficients.
