...
- **Meter resistance band boundary (2026-06-06):** The FPGA meter IC rotates through several `frame[6]` byte variants per measurement. Upper nibble `4` frames are treated as kOhm unit-normalized raw counts (`bcd_value * 0.001 kOhm`). Upper nibble `0` low-Ohm normal frames need an unrecovered factory coefficient, so custom firmware now fails them closed with `METER_REJECT_UNRESOLVED_CALIBRATION` instead of applying the old one-unit `0.0304` bench factor. Continuity marker frames remain valid/beeping.
