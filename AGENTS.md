...
- **Calibration:** ADC offset -28.0, per-channel VFP pipeline. Scope uses a 120-byte gain/offset cal table at RAM 0x20000358 (6 entries × 20 bytes, loaded from SPI flash at boot), indexed by voltage range. FUN_080018A4 (`gpio_mux_portc_porte`) reads this table to compute the scope trigger comparator DAC1 value — it is a scope trigger path, not meter cal. The 301-byte regions at state[0x356]/state[0x483] are scope roll-mode sample buffers, not calibration data. **Meter cal source is still unresolved**; the old 411-byte H2 folklore was superseded by the confirmed 115,638-byte SPI3 bulk exchange. See `analysis_v120/cal_data_myth_busted.md` for the H1 postmortem and `analysis_v120/spi3_bulk_cal_resolved.md` / `analysis_v120/h2_extracted/FINDINGS.md` for the current H2 evidence.
...
- **DAC1 is not meter calibration:** `FUN_080018A4` and repeated inline stock blocks compute/write DAC1 (`0x40007408`) from the scope calibration tables, but the recovered consumers are the scope trigger/comparator and scope autorange paths, not DMM correction. Do not fix low-DCV/current by porting a "meter DAC reference" coefficient. See `analysis_v120/meter_dac1_scope_boundary_2026_06_06.md` and `analysis_v120/cal_data_myth_busted.md`.
...
- **Meter reading pipeline working** — first live readings 2026-04-03. The current decoder extracts cross-byte BCD digits from USART2 data frames, runs a stock-style meter display state layer, rejects wrong frame families, gates AC modes on AC evidence, and applies the stock DCV decimal class selected by frame bits (`frame[8].7`, `frame[3].4`, `frame[4].4`, `frame[5].4`) after the optional `frame[2].3` raw `+10000` extension. Live fixtures on this branch include 1.5V low-DCV (`frame[8].7` class 4), 5V DCV, 32V DCV, and high-voltage/mains-class frames. See `analysis_v120/meter_stock_multiplier_tables_2026_06_05.md`, `analysis_v120/meter_math_pipeline_annotated.c`, `analysis_v120/meter_acv_stock_case_2026_06_05.md`, and `src/drivers/meter_data.c`.
- **Meter resistance band overrides (2026-04-04):** Before the fix, the display flickered between correct and wrong readings on the same resistor (e.g., a 10kΩ probe alternated between `9.821 kOhm` and `98.24 kOhm`). Root cause: the FPGA meter IC rotates through several `frame[6]` byte variants per measurement (`0x07`, `0x0A`, `0x0B`, `0x0D`, `0x0E`, `0x0F` for low-Ω; `0x40`, `0x4A`, `0x4B`, `0x4D`, `0x4E` for kΩ), each claiming a different decimal interpretation of the same BCD count. `meter_data.c` now computes resistance at the band level: upper nibble 0 of `frame[6]` → low-Ω regime → count × `0.0304 Ω`; upper nibble 4 → kΩ regime → count × `0.001 kΩ`. The `0.0304` factor is per-device/factory-calibrated and remains a bench-unit stand-in until factory calibration is loaded.
- **Meter DCV range status (2026-06-05):** The older 7V/11V limitation has been superseded for known voltage fixtures. DCV no longer derives range from `frame[6]` latch behavior or bytes `[10..11]`; it uses the stock-analysis range-bit priority plus `frame[2].3` raw extension. Live 32V frames with `frame[4].4` render about `31.96..31.98 V`, 5V frames with `frame[3].4` render about `4.994 V`, and low-DCV frames with `frame[8].7` render about `1.497..1.500 V` because stock extends `497x` counts to `1497x` and divides by `10^4`. A visually confirmed `0.200 V` input still renders about `0.4366 V` from stock-visible frame math; treat that as unresolved frontend/range/calibration behavior, not a decoder coefficient target. See `analysis_v120/dmm_state_machine_contract_2026_06_05.md`. The long-term factory calibration source is still unresolved, but no stock-only evidence supports a one-point low-voltage coefficient.
- **Boot safety system** — 3-strike crash recovery: app crashes 3x → bootloader enters SAFE MODE automatically. Hold POWER+PRM at boot = force bootloader; POWER alone is normal battery power-on and must not trigger recovery. In stock-switcher mode, the high dispatcher must wait for POWER release before jumping to stock so stock does not interpret startup POWER as shutdown. The stock hybrid builder must exact-byte NOP vanilla stock's early PC9 clear so battery-only handoff never drops power-hold before stock reasserts it. boot_validate() handshake. Never need to open case again.
