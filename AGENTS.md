...
- **Boot safety system** — 3-strike crash recovery: app crashes 3x → bootloader enters SAFE MODE automatically. Hold POWER+PRM at boot = force bootloader; POWER alone is normal battery power-on and must not trigger recovery. boot_validate() handshake. Never need to open case again.
