...
> **Before you run these:** make sure you've already run `cd firmware && make` at least once. The `option_bytes48.bin` file referenced below is **generated by the build** — it lives at `firmware/build/option_bytes48.bin` after `make` completes. It's not a file to download or find elsewhere in the repo.

Once in DFU mode, run these commands:

```bash
# 1. Set EOPB0 for 224KB SRAM (one-time — the AT32 defaults to only 96KB)
#    option_bytes48.bin is generated by `make` — run it first if you haven't.
