...
|---|---|---|
| 2026-09-22 | [Aiming the trigger](2026-09-22-aiming-the-trigger.md) | The trigger level had a proven register and no control; now MOVE hands UP/DOWN to it and the marker sits where the hardware fires (froze at code 214, predicted 207). Reg 0x02 is not an edge select and the 28-code offset is digital (EXP-55/56), so Rising/Falling became a firmware filter that refuses rather than guesses (8/8 each way on the bench; trigger modes → S2). Un-rotating the record exposed that **sample 0 of a read is often not a sample** (13/20 records), which had defeated two seam finders and one test; skipping it met the criterion (0/10 broken vs 5/10). Also: the centre servo steered on stale buffers, and the first CDC flash on unit #1 hung and was recovered with MENU held through a pinhole reset. |
| 2026-09-22 | [The read is the handshake](2026-09-22-the-read-is-the-handshake.md) | PC0 is a handover strobe on the read, not a completion flag. The FPGA self-cycles: either-edge trigger on level − 28, 512 samples after it, a ~189 ms hold, then it rolls again. The "record" several experiments and a four-day soak had built on was a phase-locked free-run snapshot. The acquisition loop became a 30 ms poll; NORMAL on slow squares stopped freezing. Postscript: FFT on live data, five display defects only a screen could show. |
| 2026-09-14 | [The pin was never silent](2026-09-14-the-pin-was-never-silent.md) | EXP-30's "PC0 never fires" was right about the fact and wrong about the cause: the trigger level sat outside the signal. With the level inside it, PC0 fires and edge-gated reads give a static body; NORMAL and SINGLE landed the same day. |
| 2026-09-14 | [CH2 was behind a flag](2026-09-14-ch2-was-behind-a-flag.md) | Both halves of CH2 were solved in August, but the boot arm sat behind a build flag defaulting to 0, so every plain coldtrace build read CH2 as zeros. Default flipped, A/B confirmed on a fresh boot. |
| 2026-09-12 | [What breaks when someone finally answers](2026-09-12-what-breaks-when-someone-finally-answers.md) | Every command this firmware ever sent the meter was discarded unparsed (wrong header); the readings were right because the meter's power-on auto mode measures volts and ohms. The meter answers traffic, not commands. |
| 2026-09-03 | [The trigger was reading the seam](2026-09-03-the-trigger-was-reading-the-seam.md) | Turned "does the trace hold still?" into a real acceptance test — both channels driven, amplitude/frequency/phase all varied, measured through the actual render path (new `spi3 frame` returns a coherent CH1+CH2 snapshot plus the renderer's own trigger offset) with an on-hardware negative control. Run 1 failed horizontal lock at 500 Hz but passed 1 kHz — the frequency-dependence was the tell. Individual frames were locking **180° out of phase**: the acquisition buffer isn't time-contiguous at its edges (stale data ~one read-cadence old at indices 32–96 / 864–928), and a trigger crossing fabricated at that seam flips the trace half a period. 1 kHz passed only because 33 ms is a whole period there. Fix: guard the trigger past the stale head (`SEAM_GUARD=128`) — jitter 10–24 → 0.01–1.0 samples, **11/11 pass**. The underlying readout-alignment defect is filed, not fixed. |
