...
| Acquisition record integrity | **S2** (EXP-42, 2026-09-14) | *needed* | **Root cause CLOSED:** the seam was the AUTO fallback reading mid-capture. PC0 is a live data-ready line (EXP-41: a read starts a capture, a reg-0x08 level crossing completes it, completion pulses PC0; the boot constant `08 AD` sat above every bench signal, which is why it "never fired"). With the level inside the signal and `fpga autowait` ≥ the capture cycle (~300 ms at 0x10), the body (samples 128–1023) is static: 0/10 gross seams vs 4/20 free-run. **New bounded defect:** the first 32–64 samples of every readout are invalid in every mode (zeros/dropout, not a rotation); `SEAM_GUARD=128` covers the display, whole-buffer consumers must skip the head. **Rate ceiling (EXP-48/49):** the FPGA will not re-arm until one fill + ~190 ms after the trigger at every timebase, so fresh triggered records top out at ~4.9/s (fast codes) / ~2.9/s (0x10); stock's 29 ms cadence is a poll of the held buffer (its June frames were untriggered), and free polling returns torn records 25–50 % of the time — keep the edge gate. S3: read the held record at edge + fill (latency), head-artifact probe |
...
| FFT + waterfall on live data | **S1** (by code, 2026-09-15, `feat/fft-live`) | [fft-live](scope/fft-live.md) | S2: bench tone test — 1 kHz at 0x10/0x0F/0x0E lands in the predicted bin ±1, one fold check, one unmeasured-code refusal; exact numbers in the spec's S2 row. **Landed:** spectrum/split/waterfall read the CH1 record (head skipped via `scope_record.h`, window spanning the record), axis in Hz only on a measured code with display and reg-0x01 agreeing, demo compiled out of live builds, `make test-fft-live` 23/23 |
...
| Trigger modes (auto / normal / single) | **S1** (EXP-47, 2026-09-14) | [trigger-modes](scope/trigger-modes.md) | S2 remaining: (a) level-vs-ADC-code transfer, (c) edge polarity. **Measured 2026-09-14 (EXP-43→47):** `fpga scope trigmode` / `fpga scope level` are the single entry points; on a fresh boot with nothing typed NORMAL captures at 0x0E/0x10/0x11/0x12 with one PC0 edge per committed record, holds at a level above the signal and resumes below it (S2b), SINGLE is one-shot 3/3, AUTO is edge-gated (1.00 edges/pair, 0/10 gross seams). Cost: the FPGA refuses to re-arm until one fill + ~200 ms after the trigger (EXP-46), so cycle time ≈ fill + 230 ms + read (~3.5 records/s at 0x10). Two wall-hits on the way: NORMAL waited for an edge before its first read (EXP-43, a read is what starts a capture), and a reg-01/08 write drops the capture in flight (EXP-47 run 1). Wishlist Tier 1 #1; milestone M2 |
