...
| Trigger level | **S2** (EXP-55/56, 2026-09-22) | [trigger-modes](scope/trigger-modes.md) | Transfer measured: the comparator fires at **reg 0x08 − 28** in record codes, on range 5 and range 7 alike (three levels each, median 28, 43/48 records within ±3), so the offset is digital and one constant places the marker. ⚠ The register is real and bench-proven, but **no control reaches it** — `scope_adjust_trigger_level()` has zero callers, so the level is immutable at runtime and the on-screen marker never moves (spec D1) |
| Trigger modes (auto / normal / single) | **S1** (EXP-47 → EXP-54, 2026-09-22) | [trigger-modes](scope/trigger-modes.md) | S2 remaining: (a) level-vs-ADC-code transfer — **DONE: the comparator sees the record + 28 on ranges 5 and 7** (EXP-53 50j, EXP-55, EXP-56: six levels, median 28, digital); (c) edge polarity — **the FPGA triggers on either edge and reg 0x02 does not select one** (EXP-53 50j; EXP-55: `02 00/01/02/03` all both-edge, 100 records, two runs; offset replicated at code 128) — the edge button is an MCU-side filter on the slope at mid-record. EXP-54 on the poll loop: NORMAL at 0x0E/0x10/0x11/0x12 advancing, holds at +100, resumes at 0, SINGLE 3/3, AUTO edge-gated; **NORMAL on a 1 Hz / 4 Hz square now triggers** (it froze on the single-arming-read images because nothing read again after the crossing). Wishlist Tier 1 #1; milestone M2 |
