...
This populates `firmware/build/` with the application binary, the bootloader, and `option_bytes48.bin` — a small generated file the first-time DFU flash needs. If you skip this step and go straight to DFU commands, you'll get "file not found" errors because `option_bytes48.bin` is a **build artifact, not a file checked into the repo.**
...
5. Set EOPB0 for 224KB SRAM (one-time). The `option_bytes48.bin` file here is generated by `make` in the previous step — it's not a download:
   ```bash
