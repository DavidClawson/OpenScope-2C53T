...
1. **Trailing clocks 64/200/512 → ZERO effect.** `0x3A` close stays `01`, `0x41` stays `80 01 C8 10`. Hypothesis ruled out.
   > **UNSUPPORTED AS WRITTEN (2026-07-28, Exp I).** The `0x41` readout used here was invalid (see note under 2), and an invalid-but-deterministic read reports "no change" whether or not anything changed. The conclusion may well hold, but it needs re-running against a readout that has been anchored to a known answer.
2. **`0x41` STATUS read = stable `80 01 C8 10` ("READY POR") across repeats** — looked like a real register.
   > **HALF-RIGHT (2026-07-28, Exp I) — then CORRECTED the same night (Exp J).** `0x8001C810` is indeed spurious: it is `0x00039020` sampled one bit early, and it sets bits 4/11/31, which are not defined Gowin bits. Exp I went further and concluded `0x00039020` was *also* noise, because it is a bit-rotation of point 3's `0xC8100001`. **That second step was wrong — the causality was inverted.** `0xC8100001` = `0x00039020` rotated left 15 bits, and Exp J established which one is the original: reading `0x11` at `/256` returns the known IDCODE `0x0120681B` perfectly aligned, so the read path is validated, and on that same validated path `0x41` returns `0x00039020`. **`0x00039020` is the genuine STATUS value; `0xC8100001` was a misaligned `/2` read of it.** Point 3 below did not find a free-running pattern that resembles the status register — it found the status register, mangled by the clock. See `expE_swd_state_diff_2026-07-28.md` §§ Experiment I, J.
...
> **⛔ REFUTED 2026-07-28 by Experiment J.** This whole paragraph rests on reads taken at `/2` (`cmd_br` defaulted to 0 until 2026-07-27), where SSPI reads are garbage. Re-run at `/256`: **`0x11` returns `0120681B 0120681B` — the IDCODE, perfectly aligned — while `0x00` returns `FF…`, `0x13` returns `00000000` and `0x41` returns `00039020`.** Four opcodes, four distinct replies. **The FPGA DOES decode SSPI opcodes and does not ignore MOSI.** The "identical MISO for every opcode" observation below was an artifact of the invalid clock, and `0xC8100001` was the STATUS register read misaligned, not a free-running pattern. Since this conclusion is what made JTAG "the route", **the JTAG rationale is substantially weakened** — SSPI reaches the config engine fine; what fails is specifically config *entry* (SYSTEM_EDIT_MODE never sets, with no CRC/BAD_COMMAND/ID_VERIFY error), which matches apicula's RECONFIG_N answer. See `expE_swd_state_diff_2026-07-28.md` § Experiment J.
